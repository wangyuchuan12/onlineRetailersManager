...
<tiles:insertDefinition name="adminLayout">
	...
	<tiles:putAttribute name="title">设备型号管理</tiles:putAttribute>
	<tiles:putAttribute name="body">
		...
		<div id="page-wrapper">
			...
			<div class="row">
				<div class="col-lg-12">
					<h1 class="page-header">设备型号管理</h1>
				</div>
			</div>
						...
						<div class="panel-body">
							...
							<div class="table-responsive">
								...
								<table class="table table-striped table-bordered table-hover"
									...
									<tbody>
										<c:forEach items="${devices}" var="device" varStatus="">
											<tr class="odd gradeX">
												<td>${device.name}</td>
												<td><joda:format value="${device.created_at}"
														pattern="yyyy-MM-dd HH:mm" /></td>
												<td><joda:format value="${device.updated_at}"
														pattern="yyyy-MM-dd HH:mm" /></td>
												<td class="center"><a
													href="<c:url value="/admin/device/update?id=${device.id}"/>">修改</a>
													<a
													href="<c:url value="/admin/device/do_delete?id=${device.id}" />">删除</a>
												</td>
											</tr>
										</c:forEach>
									</tbody>
...
function flushTable(page){
	 ...
	 window.location.href="<c:url value='/admin/device/list_page?size="+menu.val()+"&page="+page+"'/>"
}
function init(){
    ...
    menu.change(function(){
	 ...
	 var addButton = $("button[name=addButton]");
	    addButton.bind("click",function(){
	    	window.location="<c:url value='/admin/device/add'/>";
	     });
});
</script>
	</tiles:putAttribute>
